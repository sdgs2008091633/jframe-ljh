...
local login_status_model = iotservice.admin_login_info
local _M={}
--过期时间1天
...
local list=login_status_model:newsql():where_and({{'login_timestamp','<',ngx.time()},{'login_timestamp','>',(ngx.time()-86400)},{'login_token','=',ek}}):columns({'login_key','login_token','login_timestamp'}):orderby("login_timestamp"):all()
